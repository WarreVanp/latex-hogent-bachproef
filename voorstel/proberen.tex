...
\title{NLP-chatbots voor efficiëntere klantenservice: reductie van repetitieve vragen en verkorting van responstijden via accountanalyse bij Bol.com}
...
Dit bacheloronderzoek richt zich op het ontwikkelen van een **AI-gestuurd computer vision systeem** 
om de schappositie van **plant-based vleesvervangers en ontbijtgranen** in Belgische supermarkten objectief
te analyseren en de relatie te onderzoeken met hun **voedingswaarde** (Nutri-Score A-E). De Vlaamse eiwitstrategie
...
Handmatige audits van schappen zijn tijdrovend, duur en subjectief. Er bestaat geen schaalbare methode om systematisch te meten welke producten op welke hoogte en met welke zichtbaarheid aanwezig zijn, noch hoe dit samenhangt met de voedingswaarde. Dit vormt een belemmering voor:
\begin{itemize}
...
\begin{enumerate}
    ...
    \item Benchmarken en optimaliseren van object-detection en label-recognition modellen voor supermarktcondities.
    \item Valideren van nauwkeurigheid tegen handmatige ground-truth metingen.
...
Recente literatuur toont een toename van plant-based producten in Europese schappen en een groeiende vraag naar objectieve monitoringtools. Productherkenning in retail combineert vaak **beeld- en tekstinformatie** (multimodaal) om fijne onderscheidingen tussen producten te maken.

\subsection{Belangrijke bevindingen}

\begin{itemize}
    \item Multimodale benaderingen (beeld + OCR) verhogen de classificatie-robustheid in fine-grained grocery recognition \cite{Pettersson2024, Patel2025}.
    \item De **YOLO-familie** (YOLOv8) biedt een optimale trade-off tussen snelheid en nauwkeurigheid, geschikt voor real-time detectie \cite{Redmon2024}.
...
\subsection{Gelijksoortige onderzoeken en lacunes}

Er zijn studies op grocery product recognition datasets, maar weinig focussen op **plant-based producten** gecombineerd met Nutri-Score en verticale schapposities in Belgische context. Bestaande datasets missen vaak lokale merken en het specifieke labelgebruik. Dit onderzoek vult deze lacune met:
\begin{itemize}
...
\begin{enumerate}
    \item Data-acquisitie & annotatie: 4--5 weken.
    \item Modelontwikkeling (YOLOv8 detectie, label recognition, augmentatie): 5 weken.
    \item Spatial analysis & validatie: 3 weken.
    \item Visualisatie & dashboard: 2 weken.
